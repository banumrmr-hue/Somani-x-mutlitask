...
def encryption_menu_kb():
    ...
    b.row(InlineKeyboardButton(text="💎 Ultimate Encryption",    url="https://somanienc.free.nf/?i=1"))
...
def decoder_menu_kb():
    b = InlineKeyboardBuilder()
    b.row(InlineKeyboardButton(text="🔓 Decode", callback_data="dec:start"))
    b.row(InlineKeyboardButton(text="🏠 Main Menu", callback_data="menu:main"))
    return b.as_markup()
...
async def cb_enc_menu(call: CallbackQuery, state: FSMContext) -> None:
    ...
    await call.message.edit_text(
        ...
        "🤖 <b>Normal Encryption</b>\n"
        "  → Visit <b>@Somanienc_bot</b> on Telegram\n\n"
        "💎 <b>Ultimate Encryption</b>\n"
        "  → Powerful multi-layer obfuscation\n"
        "  → Anti-debug + control-flow flatten\n\n"
        "Tap a button below 👇",
...
def _auto_decode(data: bytes) -> tuple[str, bytes] | tuple[None, None]:
    """Try all decode methods in order; return (method_name, result) on first success."""
    order = ["base64", "zlib", "gzip", "hex", "base32", "base85",
             "marshal", "rot13", "url", "html", "xor", "lambda"]
    for method in order:
        try:
...
async def cb_decoder_menu(call: CallbackQuery, state: FSMContext) -> None:
    ...
    await call.message.edit_text(
        "🔓 <b>Decoder Board</b>\n\n"
        "━━━━━━━━━━━━━━━━━━━━━━\n"
        "Auto-detects encoding and decodes your file.\n\n"
        "Supports: Base64 · Base32 · Base85 · Hex\n"
        "Zlib · Gzip · Marshal · ROT13 · URL · HTML\n\n"
        "Tap <b>Decode</b> then send your encrypted file 👇",
        reply_markup=decoder_menu_kb()
    )

@router.callback_query(F.data == "dec:start")
async def cb_dec_start(call: CallbackQuery, state: FSMContext) -> None:
    await call.answer()
    await state.set_state(DecoderStates.waiting_for_input)
    await call.message.edit_text(
        "🔓 <b>Decoder Ready</b>\n\n"
        "━━━━━━━━━━━━━━━━━━━━━━\n"
        "📤 Send me your encrypted file now.\n\n"
        "I will auto-detect the encoding\n"
        "and send you the decoded file.",
        reply_markup=cancel_kb()
    )
...
async def handle_dec_file(msg: Message, state: FSMContext) -> None:
    ...
    if doc.file_size > MAX_FILE_SIZE:
        await msg.answer("⚠️ File too large (max 5 MB)."); await state.clear(); return
    pm = await msg.answer("⏳ Decoding your file…")
    try:
        data = await _dl(msg, doc)
        mn, res = _auto_decode(data)
        if mn is None:
            await msg.answer(
                "❌ <b>Could not decode.</b>\n\n"
                "File format not recognised by any of the\n"
                "supported decode methods."
            )
            return
        await msg.answer_document(
            BufferedInputFile(res, filename=f"decoded_{doc.file_name}"),
            caption=(f"✅ <b>Decoded Successfully</b>\n\n"
                     f"  Method : <code>{mn}</code>\n"
                     f"  Input  : <code>{len(data):,} bytes</code>\n"
                     f"  Output : <code>{len(res):,} bytes</code>")
        )
        await log_op(msg.from_user.id, "decoder", {"method": mn, "file": doc.file_name})
    except Exception as e:
        await msg.answer(f"❌ Error: <code>{e}</code>")
        logger.error("Decoder error: %s", e, exc_info=True)
    finally:
        await pm.delete(); await state.clear()
        await msg.answer("🔓 Decode another file?", reply_markup=back_kb("menu:decoder"))
...
async def main() -> None:
    ...
    async def on_startup() -> None:
        try:
            ...
            logger.warning("MongoDB unavailable (stats disabled): %s", e)
        me = await bot.get_me()
