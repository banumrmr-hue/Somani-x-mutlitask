...
def encryption_menu_kb():
    b = InlineKeyboardBuilder()
    b.row(InlineKeyboardButton(text="🔐 Normal Encryption",   url="https://t.me/Somanienc_bot"))
    b.row(InlineKeyboardButton(text="💎 Powerful Encryption", url="https://somanienc.free.nf/?i=1"))
...
async def cb_enc_menu(call: CallbackQuery, state: FSMContext) -> None:
    ...
    await call.message.edit_text(
        ...
        reply_markup=encryption_menu_kb()
    )
...
def _auto_decode(data: bytes):
    """Try all decode methods in order and return (method_name, result) for first success."""
    order = ["base64", "zlib", "gzip", "hex", "base32", "base85",
             "marshal", "rot13", "url", "html", "lambda", "xor"]
    for method in order:
        if method not in DECODERS:
            continue
        try:
            ok, res = decode_data(method, data)
            if ok and isinstance(res, bytes) and len(res) > 0:
                mn, _ = DECODERS[method]
                return mn, res
        except Exception:
            continue
    return None, None
...
async def cb_decoder_menu(call: CallbackQuery, state: FSMContext) -> None:
    ...
    await call.message.edit_text(
        ...
        "  5️⃣  Sending Output\n\n"
        "📥 <b>Output:</b> A <code>.py</code> file with decoded results\n\n"
        "📤 Send your <code>.py</code> file to begin.",
        reply_markup=cancel_kb()
    )
...
async def handle_dec_file(msg: Message, state: FSMContext) -> None:
    ...
    if not doc.file_name.endswith(".py"):
        await msg.answer("⚠️ Only <code>.py</code> files accepted.", reply_markup=cancel_kb())
        return
    if doc.file_size and doc.file_size > MAX_FILE_SIZE:
        await msg.answer("⚠️ File too large (max 5 MB).")
        await state.clear()
        return

    # ── Step 1: File Received ───────────────────────────────────────────────
    p1 = await msg.answer("📥 <code>File Received ...</code>")
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            raw = await _dl(msg, doc)
            src_path = tmppath / doc.file_name
            src_path.write_bytes(raw)
            original_code = src_path.read_text(encoding="utf-8", errors="replace")

            # ── Step 2: Injecting Decoder ───────────────────────────────────
            try:
                await p1.edit_text("✅ <code>File Received ✓</code>")
            except Exception:
                pass
            p2 = await msg.answer("💉 <code>Injecting Decoder ...</code>")
            await asyncio.sleep(0.4)

            injected_code = DECODER_INJECTION + original_code
            injected_path = tmppath / f"injected_{doc.file_name}"
            injected_path.write_text(injected_code, encoding="utf-8")

            try:
                await p2.edit_text("✅ <code>Decoder Injected ✓</code>")
            except Exception:
                pass

            # ── Step 3: Executing File ──────────────────────────────────────
            p3 = await msg.answer("⚙️ <code>Executing File ...</code>")

            try:
                import sys as _sys
                result = subprocess.run(
                    [_sys.executable, str(injected_path)],
                    capture_output=True,
                    text=True,
                ...
                try:
                    await p3.edit_text("⏱️ <code>Execution Timed Out</code>")
                except Exception:
                    pass
                await msg.answer(
                    f"⏱️ <b>Execution timed out</b> after <code>{EXEC_TIMEOUT}s</code>.\n"
                ...
                try:
                    await p3.edit_text("❌ <code>Execution Error</code>")
                except Exception:
                    pass
                await msg.answer(f"❌ <b>Execution error:</b>\n<pre>{exc}</pre>")
                await state.clear()
                return

            try:
                await p3.edit_text("✅ <code>Execution Complete ✓</code>")
            except Exception:
                pass

            # ── Step 4: Preparing Result ────────────────────────────────────
            p4 = await msg.answer("📝 <code>Preparing Result ...</code>")
            ...
            result_content  = "#SOMANI GOD\n" + combined_output.rstrip("\n") + "\n#SOMANI GOD\n"
            result_filename = f"decoded_{doc.file_name}"
            result_path     = tmppath / result_filename
            result_path.write_text(result_content, encoding="utf-8")

            try:
                await p4.edit_text("✅ <code>Result Prepared ✓</code>")
            except Exception:
                pass

            # ── Step 5: Sending Output ──────────────────────────────────────
            p5 = await msg.answer("📤 <code>Sending Output ...</code>")
            await asyncio.sleep(0.3)

            out_bytes = result_path.read_bytes()
            await msg.answer_document(
                BufferedInputFile(out_bytes, filename=result_filename),
                caption=(
                    f"✅ <b>Decode Complete!</b>\n\n"
                    f"━━━━━━━━━━━━━━━━━━━━━━\n"
                    f"📄 File   : <code>{doc.file_name}</code>\n"
                    f"🔢 Exit   : <code>{exit_code}</code>\n"
                    f"📏 Output : <code>{len(combined_output)} chars</code>\n"
                    f"━━━━━━━━━━━━━━━━━━━━━━\n"
                    f"⚡ <b>SOMANI DECODER</b> <code>v{BOT_VERSION}</code>"
                )
            )

            try:
                await p5.edit_text("✅ <code>Output Sent ✓</code>")
            except Exception:
                pass

            await log_op(msg.from_user.id, "decoder", {"file": doc.file_name, "exit_code": exit_code})

    except Exception as e:
        await msg.answer(f"❌ <b>Error:</b> <code>{e}</code>")
