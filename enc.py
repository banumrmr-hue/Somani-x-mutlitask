...
async def handle_dec_file(msg: Message, state: FSMContext) -> None:
    ...
    try:
        ...
        if res is None:
            await msg.answer("❌ <b>Could not decode.</b> File format not recognised."); return
        base = doc.file_name.rsplit(".", 1)[0] if "." in doc.file_name else doc.file_name
        out_name = f"decoded_{base}.py"
        await msg.answer_document(
            BufferedInputFile(res, filename=out_name),
            caption=(f"✅ <b>Decoded</b> ({mn})\n\n"
